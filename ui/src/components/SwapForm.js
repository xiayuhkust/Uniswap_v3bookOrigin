...
const SwapForm = ({ setPairs }) => {
  ...
  const metamaskInstalled = metamaskContext.status !== 'not_installed';
  ...
  useEffect(() => {
    if (typeof window.ethereum !== 'undefined') {
      ...
      // Create mock data for display when MetaMask is not installed
      const mockTokens = [
        { symbol: 'WETH', address: config.wethAddress, selected: false },
        { symbol: 'USDC', address: config.tokens[config.wethAddress].address, selected: false }
      ];
  ...
  return (
    <section className="SwapContainer">
      ...
      {metamaskContext.status === 'not_installed' && (
        <div className="MetaMaskWarning" style={{ padding: '20px', margin: '20px 0', backgroundColor: '#ffeeee', borderRadius: '8px', textAlign: 'center' }}>
          <h3>MetaMask Not Installed</h3>
          <p>This application requires MetaMask to function properly. Please install MetaMask to use all features.</p>
          <a href="https://metamask.io/download/" target="_blank" rel="noopener noreferrer" style={{ color: '#1a73e8', textDecoration: 'underline' }}>
            Download MetaMask
          </a>
        </div>
      )}
      ...
      {path ?
        ...
        <div style={{ padding: '20px', textAlign: 'center' }}>
          {metamaskContext.status !== 'not_installed' ? (
            ...
            <span>Please install MetaMask to view pairs and use the application.</span>
          )}
        </div>
      }
    </section>
